...
int main(int argc, char **argv) {
  ...
  if ((argc != 3 && argc != 4) || a1 == "-h" || a1 == "--help") {
    ...
    cout << "\nUsage: " << self
         << " INPUTFILE OUTPUT[FORMAT|FILE] [CONFIGFILE]\n"
  ...
  if (outputExt == "csv")
    generator = new GeneratorCSV;
  else if (outputExt == "svg")
    generator = new GeneratorSVG;
  else if (outputExt == "conf")
    generator = new GeneratorCONF;
  else if (outputExt == "png")
    generator = new GeneratorPNG;
  else {
...
Date::Date(int tyear, int tmonth) { *this = Date(tyear, tmonth, 0); }
Date::Date(int tyear) { *this = Date(tyear, 0, 0); }
Date::Date() { *this = Date(0, 0, 0); }
...
bool Date::operator==(Date d) { return (!(*this < d) && !(d < *this)); }
